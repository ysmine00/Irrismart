...
% ── Bibliography handled via \begin{thebibliography} at end of document ───────
...
\vfill
% \includegraphics[width=3.5cm]{aui_logo.png}
\end{titlepage}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  box/.style={rectangle, rounded corners=4pt, draw=auiblue, thick,
              minimum height=1.1cm, minimum width=3.0cm,
              align=center, font=\small\linespread{0.9}\selectfont},
  tier/.style={rectangle, draw=gray!60, dashed, rounded corners=6pt,
               inner sep=8pt},
  tierlabel/.style={font=\small\bfseries, text=gray!80},
  arr/.style={-{Stealth[length=5pt]}, thick, auiblue},
  node distance=0.5cm,
]

%% ── FIELD TIER (left column) ────────────────────────────────────────────────
\node[box] (s1) {Sensor ID010001\\{\tiny Olive, GPIO32}};
\node[box, below=0.4cm of s1] (s2) {Sensor ID010002\\{\tiny Citrus, GPIO33}};
\node[box, below=0.4cm of s2] (s3) {Sensor ID010003\\{\tiny Wheat, GPIO34}};
\node[box, below=0.4cm of s3, fill=auiblue!8] (gw)
      {Heltec WiFi LoRa 32 V3\\{\tiny Gateway · 191 lines C++}};

\node[tierlabel, above=0.5cm of s1] (ft) {Field Tier};
\begin{pgfonlayer}{background}
  \node[tier, fit=(ft)(s1)(s2)(s3)(gw)] {};
\end{pgfonlayer}

%% ── CLOUD TIER (middle column) ──────────────────────────────────────────────
\node[box, right=2.8cm of s1, fill=auiblue!8] (api)
      {Flask REST API\\{\tiny 30 endpoints · Python 3.11}};
\node[box, below=0.4cm of api] (db)
      {PostgreSQL\\{\tiny 8 tables · Railway}};
\node[box, below=0.4cm of db, fill=auiblue!8] (ml)
      {RF + IsolationForest\\{\tiny scikit-learn 1.3.2}};
\node[box, below=0.4cm of ml] (wx)
      {Open-Meteo\\{\tiny Weather cache · 6h TTL}};

\node[tierlabel, above=0.5cm of api] (ct) {Cloud Tier};
\begin{pgfonlayer}{background}
  \node[tier, fit=(ct)(api)(db)(ml)(wx)] {};
\end{pgfonlayer}

%% ── USER TIER (right column) ────────────────────────────────────────────────
\node[box, right=2.8cm of api, fill=auiblue!8] (dash)
      {PWA Dashboard\\{\tiny Bilingual FR/AR · 2109 lines}};
\node[box, below=0.4cm of dash] (sms)
      {Twilio SMS + WhatsApp\\{\tiny 7 alert types}};
\node[box, below=0.4cm of sms] (chat)
      {Claude Haiku\\{\tiny Darija chatbot}};

\node[tierlabel, above=0.5cm of dash] (ut) {User Tier};
\begin{pgfonlayer}{background}
  \node[tier, fit=(ut)(dash)(sms)(chat)] {};
\end{pgfonlayer}

%% ── ARROWS ──────────────────────────────────────────────────────────────────
\draw[arr] (s1.east) -| ++(0.6,0) |- (gw.west);
\draw[arr] (s2.east) -| ++(0.6,0) |- (gw.west);
\draw[arr] (s3.east) -| ++(0.6,0) |- (gw.west);
\draw[arr] (gw.east) -- node[above,font=\tiny,auiblue]{HTTP POST}
           (gw.east -| api.west) -- (api.west|-gw);
\draw[arr] (api.south) -- (db.north);
\draw[arr] (db.south) -- (ml.north);
\draw[arr] (ml.south) -- (wx.north);
\draw[arr] (api.east) -- node[above,font=\tiny,auiblue]{JSON}
           (dash.west|-api);
...
\begin{itemize}
  \item \textbf{Makerfabs packet ingestion:} Raw string ``ID010003 REPLY: SOIL INDEX:0 H:48.85 T:30.50 ADC:896 BAT:1016'' correctly parsed and stored; ADC=896 \ensuremath{\rightarrow} moisture=20.8\% via Equation~\ref{eq:adc}.
...
\begin{enumerate}
  ...
  \item \textbf{Direct ADC mode:} The current demonstration deploys sensors via direct ADC connection rather than LoRa radio. The full two-hop architecture (sensor node \ensuremath{\rightarrow} LoRa \ensuremath{\rightarrow} gateway \ensuremath{\rightarrow} HTTP \ensuremath{\rightarrow} cloud) has not been validated end-to-end in the field.

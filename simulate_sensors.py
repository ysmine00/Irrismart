...
API = "https://irrismart.up.railway.app"
...
while True:
    ...
    for s in SENSORS:
        # Each sensor always sends its own fixed crop
        ...
        try:
            r = requests.post(f"{API}/api/data", json=payload, timeout=10)
            status = "✅" if r.status_code == 200 else f"❌ {r.status_code}"
            print(f"  [{s['crop'].upper()}] {s['name']}: moisture={moisture}% temp={temp}°C rain={rain}mm bat={battery}% → {status}")
